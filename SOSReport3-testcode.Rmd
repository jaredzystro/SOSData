...
#packages <- c("survey","ggplot2","shiny","plyr","dplyr","srvyr", "sf", "spData","stringr","maps","ggiraph","shinycssloaders")
...
# Change non-responses to NA
data$GrowForage <- ifelse (data$Q07 == "No answer" | data$Q07 == "partial complete" | data$Q07 == "skipped due to branching", NA, data$Q08)
# Convert from text to numeric -- 100 for field, 0 for not. Probably a better way to do this...
data$GrowForage <- ifelse (data$GrowForage == "Some annual forage crop acres" | data$GrowForage == "Some annual forage crop acres:", 100, 0) %>% as.numeric()
...
data$OnlyVeg <- ifelse(data$GrowVegis == 100 & data$GrowField == 0, TRUE, FALSE)
# Dropping cover crops for now
# data$HasCover <- ifelse(data$GrowCover == 100, TRUE, FALSE)
# data$OnlyCover <- ifelse(data$GrowCover == 100 & data$GrowVegis == 0 & data$GrowField == 0, TRUE, FALSE)
data$HasField <- ifelse(data$GrowField == 100, TRUE, FALSE)
data$OnlyField <- ifelse(data$GrowField == 100 & data$GrowVegis == 0, TRUE, FALSE)

data2015$HasVeg <- ifelse(data2015$GrowVegis == 100, TRUE, FALSE)
data2015$OnlyVeg <- ifelse(data2015$GrowVegis == 100 & data2015$GrowField == 0, TRUE, FALSE)
#data2015$HasCover <- ifelse(data2015$GrowCover == 100, TRUE, FALSE)
#data2015$OnlyCover <- ifelse(data2015$GrowCover == 100 & data2015$GrowVegis == 0 & data2015$GrowField == 0, TRUE, FALSE)
data2015$HasField <- ifelse(data2015$GrowField == 100, TRUE, FALSE)
data2015$OnlyField <- ifelse(data2015$GrowField == 100 & data2015$GrowVegis == 0, TRUE, FALSE)

# No cover crop question for 2010
data2010$HasVeg <- ifelse(data2010$GrowVegis == 100, TRUE, FALSE)
data2010$OnlyVeg <- ifelse(data2010$GrowVegis == 100 & data2010$GrowField == 0, TRUE, FALSE)
data2010$HasField <- ifelse(data2010$GrowField == 100, TRUE, FALSE)
data2010$OnlyField <- ifelse(data2010$GrowField == 100 & data2010$GrowVegis == 0, TRUE, FALSE)
...
# Change non-responses to NA
data$NoOGSeedProcessor <- ifelse (data$Q29A == "No answer" | data$Q29A == "partial complete" | data$Q29A == "skipped due to branching", NA, data$Q29A)
# Order factors
...
# Change non-responses to NA
data$NoOGSeedQuant <- ifelse (data$Q29A == "No answer" | data$Q29A == "partial complete" | data$Q29A == "skipped due to branching", NA, data$Q29A)
# Order factors
...
# Change non-responses to NA
data$NoOGSeedSave <- ifelse (data$Q29A == "No answer" | data$Q29A == "partial complete" | data$Q29A == "skipped due to branching", NA, data$Q29A)
# Order factors
...
# Change non-responses to NA
data$NoOGSeedQual <- ifelse (data$Q29A == "No answer" | data$Q29A == "partial complete" | data$Q29A == "skipped due to branching", NA, data$Q29A)
# Order factors
...
### 29F Price ----
# Change non-responses to NA
data$NoOGSeedPrice <- ifelse (data$Q29A == "No answer" | data$Q29A == "partial complete" | data$Q29A == "skipped due to branching", NA, data$Q29A)
# Order factors
...
# Change non-responses to NA
data$NoOGSeedVar <- ifelse (data$Q29A == "No answer" | data$Q29A == "partial complete" | data$Q29A == "skipped due to branching", NA, data$Q29A)
# Order factors
...
# Change non-responses to NA
data$NoOGSeedTrait <- ifelse (data$Q29A == "No answer" | data$Q29A == "partial complete" | data$Q29A == "skipped due to branching", NA, data$Q29A)
# Order factors
...
# scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = wrap_width))
  ...
  # Add n if checked
  if (input$show_n) {
    p <- p + geom_text(aes(label=summary_labels, vjust=4), color="black", size=LABELSIZE, position = position_dodge(width = 0.9))
  }
...
#    ylim(0,100) +
  ...
  if (input$show_n) {
    p <- p + geom_text(aes(label=summary_labels, vjust=1), color="black", size=LABELSIZE, position = position_dodge(width=0.9))
  }
...
GradientMap <- function (input, question, title, catlab, year) {
